...
fn run(matrix : &cv::Mat, cursor : &Cursor) -> Result<(), String> {
    let mut classifier =
    ...
    loop {
        if try!(capture.grab()) {
            ...
            try!(imgproc::resize(&image0, &mut image1, dest, 0.0, 0.0, 0));
        
            try!(highgui::imshow("midicam", &image1));
...
fn main() {
	...
	thread::spawn(move || {
		...
		loop {
			//let note = *pix / 2;
			let mut row = the_matrix.ptr(cursor.y).unwrap();
			unsafe {
